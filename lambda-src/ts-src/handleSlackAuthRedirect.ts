...
export async function handleSlackAuthRedirect(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
    ...
    const slackClientId = await getSecretValue('Selector', 'slackClientId');
    const slackClientSecret = await getSecretValue('Selector', 'slackClientSecret');
    ...
    let successText = `Successfully installed Selector in workspace`;
    if(data.team?.name) {
      successText = `Successfully installed Selector in workspace ${data.team.name}`;
    }
    else if(data.enterprise?.name) {
      successText = `Successfully installed Selector in organisation ${data.enterprise.name}`;
    } 
